...
name = 'cilindro_non_inclinato_lambda'+lifetime+'_exp'+exp+'_'
t0 = 0
...
for j in range(lunghezza_verticale):
	for i in range(len(angle)-1):
		...
		X_rot = rotation.apply(Xinit) + np.random.normal(0,0.05,len(Xinit)) +[0,0,j*0.1]
...
for j in range(lunghezza_verticale*10):
	np.random.seed(j)
	X1_i =  [0,0,j*0.01] + np.random.multivariate_normal(mean,cov)
	X1.append(X1_i)
y1 = np.ones(len(X1))	
