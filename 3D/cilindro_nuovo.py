...
name = 'cilindro_nuovo_lambda'+lifetime+'_exp'+exp+'_'
t0 = 0
...
np.random.seed(2)
x = 0.5*np.cos(angle) + np.random.normal(0,0.05,100)
np.random.seed(3)
y = 0.5*np.sin(angle) + np.random.normal(0,0.05,100)
...
z1 = np.random.uniform(low=0, high=1, size=50)
...
dat = datasets.make_blobs(n_samples=[50], n_features=3, cluster_std=0.1,center_box=(1,1,1), random_state=0, return_centers=True)
X2 = dat[0]
Y2 = dat[1]
