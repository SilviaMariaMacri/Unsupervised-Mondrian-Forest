...
np.random.seed(1)
phi = np.random.uniform(low=0, high=np.pi, size=100)


np.random.seed(3)
eps1 = np.random.normal(0,0.05,100)
np.random.seed(4)
eps2 = np.random.normal(0,0.05,100)
np.random.seed(5)
eps3 = np.random.normal(0,0.05,100)
...
blob = datasets.make_blobs(n_samples=50,n_features=3,centers=[(0,0,0)],cluster_std=0.025,random_state=18)
Xb = blob[0]
yb = np.ones(len(Xb))
