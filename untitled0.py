...
def Cut_with_data(data,l,dist_matrix):
	...
	return point1,point2,lati,matrix
...
def MondrianPolygon_SingleCut(data,t0,l,lifetime,father,dist_matrix,neighbors):
	...
	return risultato, point1, point2, lati
...
def MondrianPolygon(X,t0,lifetime,dist_matrix): 
	...
	for i in m:
		...
		try:
			...
			mondrian,point1,point2,lati = MondrianPolygon_SingleCut(i[2],i[0],i[1],lifetime,i[3],dist_matrix,i[4])
				...
				for k in range(len(h[4])):
					...
					for n in range(len(i[4])):
						if i[4][n] != h[3]:
							continue
						if n==lati[0]:
							segmento1 = [point[j],i[1][lati[j]][0]]    
							segmento2 = [i[1][lati[j]][1],point[j]]
							neigh1 = m[-2][3]
							neigh2 = m[-1][3]
							
							h[1][k] = segmento2
							h[1].insert(k+1,segmento1)
							h[4][k] = neigh2
							h[4].insert(k+1,neigh1)
					...
					#for j in range(2):
					#	if i[4][lati[j]] == h[3]:
							#segmento1 = [h[1][k][0],point[j]]
							#segmento2 = [point[j],h[1][k][1]]
					#		segmento1 = [point[j],i[1][lati[j]][0]]    
					#		segmento2 = [i[1][lati[j]][1],point[j]]
					#		if j==0:
					#			neigh1 = m[-2][3]
		...
		#	for j in range(2):
		#		if i[4][lati[j]] != np.nan: #lato tagliato deve avere vicini
		#			#	continue
		#			for h in m[:-2]:
		#				if h[3] != i[4][lati[j]]:
		#					continue
					#à	for k in range(len(h[4])): #per ogni elemento vicino
					#		if h[4][k] != i[3]:
					#			continue
					#		segmento1 = [h[1][k][0],point[j]]
					#		segmento2 = [point[j],h[1][k][1]]
